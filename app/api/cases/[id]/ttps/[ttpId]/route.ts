...
export async function DELETE(
  _req: NextRequest,
  { params }: { params: { id: string; ttpId: string } }
) {
  ...
  const ttp = await prisma.ttp.findUnique({
    where: { id: params.ttpId },
    select: { caseId: true },
  });
  if (!ttp || ttp.caseId !== params.id) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  await prisma.ttp.delete({ where: { id: params.ttpId } });
